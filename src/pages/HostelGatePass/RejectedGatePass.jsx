...
export default function RejectedGatePass() {
    const columns = [
        {
            field: 's_no',
            headerName: 'S.No.',
        ...
        const fetchData = async () => {
            try {
                const response = await axiosPrivate.get(
                    `${API_BASE_URL}/student/${auth.user?.id}/hostel/rejected_gate_pass`
                )
                console.log("API response:=====>>", response?.data?.data?.rejectedGatePass)
                if (!response.data) {
                    throw new Error('Response Data not found ..!')
                }

                const newData = response?.data?.data?.rejectedGatePass.map(
                    (item, index) => ({
                        ...item,
                        s_no: index + 1,
                    })
